...
function AddDeviceModal({ onClose, onSuccess }) {
    ...
    const handleSubmit = async () => {
        ...
        for (let i = 0; i < statuses.length; i++) {
            ...
            setConnectionStatus(prev => [...prev, { text: statuses[i], done: i < 1 }]);
        }
                        ...
                        <div className="w-full flex flex-col gap-3">
                            {connectionStatus.map((s, i) => (
                                <div key={i} className="flex items-center gap-2 text-sm">
                                    <span className={s.done ? "text-green-500" : "text-yellow-500"}>
                                        {s.done ? "✓" : "⏳"}
                                    </span>
                                    <span className={s.done ? "text-gray-700" : "text-gray-400"}>{s.text}</span>
                                </div>
                            ))}
                        </div>
                    </div>
                ...
                {step === 3 && (
                    <div className="flex flex-col items-center py-6 gap-4">
                        ...
                        <div className="text-center">
                            <h3 className="font-bold text-gray-800 mb-1">{form.nickname}</h3>
                            <p className="text-sm text-gray-400">PLANTI-{form.serial}</p>
                        </div>
                        <div className="w-full flex flex-col gap-2 text-sm text-gray-500 bg-green-50 rounded-lg p-3">
                            <div className="flex items-center gap-2 text-green-600"><span>✓</span> 기기 신호 탐색 완료</div>
                            <div className="flex items-center gap-2 text-yellow-500"><span>⏳</span> 서버 연결 확인 중...</div>
                            <div className="flex items-center gap-2 text-gray-400"><span>○</span> 기기 생태 정보 수집 중...</div>
                            <div className="flex items-center gap-2 text-gray-400"><span>○</span> 페이지로 이동 중...</div>
                        </div>
                        <button
